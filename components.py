...
def display_app_sidebar():
    ...
    with st.sidebar:
        ...
        display_select_mode()
        st.divider()
        display_initial_sidebar_message()
...
def display_initial_sidebar_message():
    # 「社内文書検索」の機能説明
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            ...
                            display_text = f"{message['content']['main_file_path']}（ページNo.{page_number}）"
                        else:
                            display_text = message["content"]["main_file_path"]
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    ...
                                    display_text = f"{sub_choice['source']}（ページNo.{page_number}）"
                                else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        main_file_path = main_metadata["source"]
        ...
        if "page" in main_metadata:
            # ページ番号を取得
            main_page_number = main_metadata["page"]
            # 「メインドキュメントのファイルパス」と「ページ番号」を表示
            display_text = f"{main_file_path}（ページNo.{main_page_number + 1}）"
        else:
